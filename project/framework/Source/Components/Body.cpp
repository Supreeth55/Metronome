...
void Body::Integrate(float DT)
{
	...
	if (mIsPhysicsObject)
	{
		...
		if (body_State == ON_SURFACE && (mpEnowner->GetEntityType() == ENTITY_TYPE::PLAYER || mpEnowner->GetEntityType() == ENTITY_TYPE::COLLIDER || mpEnowner->GetEntityType() == ENTITY_TYPE::ENEMY || 
			 mpEnowner->GetEntityType() == ENTITY_TYPE::WENEMY ||mpEnowner->GetEntityType() == ENTITY_TYPE::BLOCK))
		{
			mVelo_z = 0;
		}
		...
		//CollisionManager::GetInstance()->mSpatial->AssignCell(mShape, mPos_x, mPos_y);
		//Transform* pTr1 = (*MemoryManager::GetInstance()->ObjectPool)[key].pTr;

		//std::cout << "PTR x y z: " << pTr1->mPositionX << " " << pTr1->mPositionY << " " << pTr1->mPositionZ << std::endl;
		//std::cout << "pBd x y z: " << mPos_x << " " << mPos_y << " " << mPos_z << std::endl;
	}
}
