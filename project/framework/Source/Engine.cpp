...
bool Engine::Initialize(int type)
{
	...
	DXInputManager::GetInstance()->Initialize(mHinstance, mHWND, screenWidth, screenHeight);
	result = GraphicsManager::GetInstance()->Initialize(screenWidth, screenHeight, mHWND);
	if (!result)
	{
		return false;
	}
	...
	if (type == 0)
	{
		pGSM->PushState(statetemp, "Level1");
	}
	if (type == 1)
	{
		pGSM->PushState(statetemp, "Level2");
	}
	if (type == 2)
	{
		pGSM->PushState(statetemp, "Level3");
	}
