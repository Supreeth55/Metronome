...
LightList::LightList() : Entity(ENTITY_TYPE::LIGHTLIST)
{
}

LightList::~LightList() {}

void LightList::Serialize(Json::Value root, unsigned int position)
{
	...
	(*MemoryManager::GetInstance()->ObjectPool)[position].pEn = this;
	(*MemoryManager::GetInstance()->ObjectPool)[position].pEn->mEntityName = "LightList";
	(*MemoryManager::GetInstance()->ObjectPool)[position].pEn->complete = true;
	key = position;
}

void LightList::Update(float deltaTime)
{
	if (pLd->pLt)
	{
		pLd->pLt->SetPosition(pTr->mPositionX, pTr->mPositionY, pTr->mPositionZ);
