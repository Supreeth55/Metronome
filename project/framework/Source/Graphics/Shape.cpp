...
FBX::FBX(string filename, const vector<string>& animationnamelist)
{
	...
	//mSimpleVShader->Use();
	AddBoneNodeToArray(mRootBone, LBoneTr, Matrix::CreateScale(mModelSize));
	AddBoneToArray(mRootBone, LBonePnt, Matrix::CreateScale(mModelSize));//for debug
...
void FBX::Update(float time, string animationName, bool isLoop, float speed)
{
	...
	AddBoneNodeToArray(mRootBone, LBoneTr, mModelTr);
...
*/
void FBX::AddBoneNodeToArray(BoneNode & rootnode, vector<Matrix>& jointMatrices, Matrix parentBindTransform)
{
	string name = LBoneName[rootnode.mIndex];
	Matrix bindTransform;
	if (LAnimationlist[mCurrentAnimation].getChennelList().find(name) == LAnimationlist[mCurrentAnimation].getChennelList().end())
	{
		bindTransform = parentBindTransform * rootnode.GetLocalBindTransform();
	}
	else
	{
		bindTransform = parentBindTransform * LAnimationlist[mCurrentAnimation].getChennelList()[name].tran;
	}
	rootnode.getGlobalBindTransform() = bindTransform;
	jointMatrices[rootnode.mIndex] = bindTransform * LBoneOffsetMatrix[rootnode.mName];

	for (int i = 0; i < rootnode.LChildren.size(); i++)
	{
		AddBoneNodeToArray(rootnode.LChildren[i], jointMatrices, bindTransform);
	}
}
/**
add Bone Node To Array for debug
*/
void FBX::AddBoneToArray(BoneNode & rootnode, vector<Matrix>& jointMatrices, Matrix parentBindTransform)
{
	string name = LBoneName[rootnode.mIndex];
	Matrix bindTransform;
	if (LAnimationlist[mCurrentAnimation].getChennelList().find(name) == LAnimationlist[mCurrentAnimation].getChennelList().end())
	...
	jointMatrices[rootnode.mIndex] = bindTransform;

	for (int i = 0; i < rootnode.LChildren.size(); i++)
	{
		AddBoneToArray(rootnode.LChildren[i], jointMatrices, bindTransform);
	}
}
/**
Set BoneInfo
