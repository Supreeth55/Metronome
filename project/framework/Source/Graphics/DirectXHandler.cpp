	...
	float rScreenDepth, float rScreenNear)
{
	...
	padapter_output->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &num_modes, NULL);
	DXGI_MODE_DESC* pdisplay_mode_list;
	pdisplay_mode_list = new DXGI_MODE_DESC[num_modes];
	result = padapter_output->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &num_modes, pdisplay_mode_list);

	//check monitor is supported
	for (size_t i = 0; i < num_modes; i++)
	{
		if (pdisplay_mode_list[i].Width == (unsigned int)rScreenWidth)
		{
			if (pdisplay_mode_list[i].Height == (unsigned int)rScreenHeight)
			{
	...
	float screen_aspect = (float)rScreenWidth / (float)rScreenHeight;
	...
	mOrthoMatrix = Matrix::CreateOrthographic((float)rScreenWidth, (float)rScreenHeight, rScreenNear, rScreenDepth);
...
void DirectXHandler::ResizeScreen(int rScreenWidth, int rScreenHeight)
{
	...
	result = pSwapChain->ResizeBuffers(1, mScreenWidth, mScreenHeight, DXGI_FORMAT_R8G8B8A8_UNORM, 0);
	...
	mOrthoMatrix = Matrix::CreateOrthographic(float(rScreenWidth), float(rScreenHeight), mScreenNear, mScreenDepth);
}
